...
You have access to the PagerDuty REST API V2 for incident management and operations.
...
- Provide analytics, metrics, and operational insights
...
7. For analytics and metrics, present data in a clear, tabular format

**Available Operations (use the corresponding tools):**
- List/search incidents with filters (status, urgency, service, team)
- Get detailed incident information including log entries and timeline
- Acknowledge incidents
- Resolve incidents with resolution notes
- Add notes to incidents
- Get related incidents for an incident
- Get past similar incidents (AIOps)
...
- Get incident analytics (MTTA, MTTR, counts)
- List maintenance windows
- List recent change events / deployments
...
Always be helpful, accurate, and action-oriented."""
    ...
    async def _list_incidents(self, filter_query: str = "") -> str:
        ...
        IMPORTANT: Uses team_ids only (no service_ids) for PagerDuty API v2 compatibility.
        service_ids combined with team_ids causes errors on many PagerDuty environments.
        """
        try:
            team_id = get_team_id()
            
            # Get last 30 days of data for broader view
            ...
            # Null-safe: PagerDuty API v2 may return null for incidents array
            incidents = (result.get("incidents") if isinstance(result, dict) else None) or []
            if not incidents:
                return f"✅ No incidents found for {get_team_name()} team in the last 30 days."
            ...
            if triggered:
                ...
                for inc in triggered[:15]:
                    urgency_emoji = "🔥" if inc.get('urgency') == 'high' else "⚠️"
                    svc = inc.get('service') or {}
                    created = (inc.get('created_at') or '')[:16].replace('T', ' ')
                    output += f"{urgency_emoji} **{(inc.get('title') or 'No title')[:70]}**\n"
                    output += f"   `{inc.get('id', 'N/A')}` | {svc.get('summary', 'Unknown Service')} | {created}\n"
                output += "\n"
            
            if acknowledged:
                output += f"### 🟡 Acknowledged ({len(acknowledged)})\n"
                for inc in acknowledged[:10]:
                    svc = inc.get('service') or {}
                    output += f"• **{(inc.get('title') or 'No title')[:60]}**\n"
                    output += f"  `{inc.get('id', 'N/A')}` | {svc.get('summary', 'Unknown Service')}\n"
                output += "\n"
            
            if resolved:
                output += f"### 🟢 Recently Resolved ({len(resolved)})\n"
                for inc in resolved[:10]:
                    resolved_at = (inc.get('last_status_change_at') or '')[:16].replace('T', ' ')
                    output += f"• [{inc.get('status', '').upper()}] {(inc.get('title') or 'No title')[:50]} (`{inc.get('id', 'N/A')}`) — {resolved_at}\n"
    ...
    async def _get_incident_details(self, incident_id: str) -> str:
        """Get comprehensive incident details including alerts with custom details.
        
        Fetches incident data, alerts (with full body/labels/annotations),
        notes, and log entries in parallel — matching PagerDuty portal detail level.
        """
        try:
            import asyncio
            
            async def safe_fetch(tool_name, params):
                try:
            ...
            inc_task = safe_fetch("get_incident_details", {"incident_id": incident_id})
            alerts_task = safe_fetch("list_incident_alerts", {"incident_id": incident_id, "limit": 50})
            notes_task = safe_fetch("get_incident_notes", {"incident_id": incident_id})
            log_task = safe_fetch("get_incident_log_entries", {"incident_id": incident_id})
            
            results = await asyncio.gather(inc_task, alerts_task, notes_task, log_task)
            inc_data, alerts_data, notes_data, log_data = results
            
            incident = inc_data.get("incident") or inc_data or {}
            ...
            if not incident.get('id') and not incident.get('title'):
                return f"❌ Could not find incident `{incident_id}`. Please verify the ID."
            ...
            output += f"**Status:** {(incident.get('status') or 'N/A').upper()}\n"
            output += f"**Urgency:** {urgency_emoji} {(incident.get('urgency') or 'N/A').upper()}\n"
            output += f"**Service:** {(incident.get('service') or {}).get('summary', 'N/A')}\n"
            ...
            if incident.get("assignments"):
                assignees = [(a.get('assignee') or {}).get('summary', 'Unknown') for a in incident.get('assignments', [])]
                output += f"\n**Assigned to:** {', '.join(assignees)}\n"
            
            if incident.get('escalation_policy'):
                output += f"**Escalation Policy:** {(incident.get('escalation_policy') or {}).get('summary', 'N/A')}\n"
            
            if incident.get('html_url'):
                output += f"\n🔗 **PagerDuty URL:** {incident.get('html_url')}\n"
            
            # ===== ALERTS with Custom Details (matching PagerDuty portal) =====
            if alerts:
                output += f"\n---\n### 🔔 Alerts ({len(alerts)})\n\n"
                for alert in alerts[:10]:
                    ...
                    alert_emoji = "🔴" if alert_status == 'triggered' else "🟡" if alert_status == 'acknowledged' else "🟢"
                    
                    output += f"{alert_emoji} **Alert `{alert.get('id', 'N/A')}`** — {alert.get('severity', 'N/A').upper()}\n"
                    output += f"   Summary: {(alert.get('summary') or 'N/A')[:100]}\n"
                    output += f"   Created: {(alert.get('created_at') or 'N/A')[:19].replace('T', ' ')}\n"
                    
                    if alert.get('alert_key'):
                        output += f"   Alert Key: `{alert.get('alert_key')[:60]}...`\n"
                    
                    # Parse alert body / custom details
                    ...
                    if isinstance(details, dict):
                        ...
                        if isinstance(labels, dict) and labels:
                            output += "\n   **Labels:**\n"
                            for k, v in list(labels.items())[:12]:
                                output += f"   - {k} = {v}\n"
                        
                        # Firing info (text block)
                        firing = details.get('firing') or ''
                        if isinstance(firing, str) and firing:
                            output += f"\n   **Firing Details:**\n"
                            for line in firing.split('\n')[:15]:
                                output += f"   {line}\n"
                        ...
                        if isinstance(annotations, dict) and annotations:
                            ...
                            for k, v in annotations.items():
                                output += f"   - {k} = {str(v)[:150]}\n"
                        
                        # Runbook URL
                        runbook = (
                            details.get('runbook_url') or 
                            (annotations.get('runbook_url') if isinstance(annotations, dict) else '') or 
                            ''
                        )
                        if runbook:
                            output += f"\n   📖 **Runbook:** {runbook}\n"
                        ...
                        # Source graph
                        source = details.get('source') or details.get('Source') or ''
                        if source:
                            output += f"   📊 **Source:** {str(source)[:100]}...\n"
                        
                        # Firing/Resolved counts
                        num_firing = details.get('num_firing')
                        num_resolved = details.get('num_resolved')
                        if num_firing is not None or num_resolved is not None:
                            output += f"   **Firing:** {num_firing or 0} | **Resolved:** {num_resolved or 0}\n"
                        
                        # Silence URL
                        silence_url = details.get('SilenceURL') or details.get('silence_url') or ''
                        if silence_url:
                            output += f"   🔇 **Silence URL:** {silence_url}\n"
                    ...
                    for ctx in contexts[:3]:
                        if isinstance(ctx, dict):
                            output += f"   🔗 [{ctx.get('text', 'Link')}]({ctx.get('href', '')})\n"
            ...
            if notes:
                ...
                for note in notes[:10]:
                    if not isinstance(note, dict):
                        continue
                    user = (note.get('user') or {}).get('summary', 'Unknown')
                    created = (note.get('created_at') or '')[:19].replace('T', ' ')
                    output += f"**{user}** ({created}):\n> {note.get('content', '')}\n\n"
            
            # ===== TIMELINE (last 5 entries) =====
            if log_entries:
                output += f"---\n### 📜 Recent Timeline ({len(log_entries)} entries)\n\n"
                for entry in log_entries[:5]:
                    if not isinstance(entry, dict):
                        continue
                    created = (entry.get('created_at') or '')[:19].replace('T', ' ')
                    entry_type = (entry.get('type') or 'unknown').replace('_', ' ').title()
                    output += f"• **{created}** — {entry_type}: {entry.get('summary', 'Action performed')}\n"
            
            # Action hints
            if incident.get('status') == 'triggered':
                output += f"\n\n💡 *Actions available: acknowledge, resolve, add note, view timeline*"
            elif incident.get('status') == 'acknowledged':
                output += f"\n\n💡 *Actions available: resolve, add note, view timeline*"
    ...
    async def _get_related_incidents(self, incident_id: str) -> str:
        """Get related incidents (requires PagerDuty AIOps feature)."""
        try:
            result = await self.mcp_client.execute_tool("get_related_incidents", {"incident_id": incident_id})
            
            if not result or not isinstance(result, dict):
                return f"No related incidents data available for `{incident_id}`. This feature may require PagerDuty AIOps."
            
            related = result.get("related_incidents") or []
            ...
            for inc in related[:10]:
                if not isinstance(inc, dict):
                    continue
                status_emoji = {"triggered": "🔴", "acknowledged": "🟡", "resolved": "🟢"}.get(inc.get('status', ''), "⚪")
                svc = (inc.get('service') or {}).get('summary', 'Unknown Service')
                rel_type = inc.get('relationship', 'related')
                output += f"{status_emoji} **{(inc.get('title') or 'No title')[:60]}**\n"
                output += f"  `{inc.get('id', 'N/A')}` | {inc.get('status', 'unknown')} | {svc} | Type: {rel_type}\n\n"
            ...
            if '404' in error_msg or 'Not Found' in error_msg:
                return f"ℹ️ Related incidents feature is not available for `{incident_id}`. This requires PagerDuty AIOps add-on."
            return f"❌ Error getting related incidents: {error_msg}"
    ...
    async def _get_oncalls(self) -> str:
        ...
        Null-safe: handles case where PagerDuty API returns None or
        nested objects have null fields.
        """
        try:
            result = await self.mcp_client.execute_tool("list_oncalls", {})
            
            # Null-safe: result itself can be None
            if not result or not isinstance(result, dict):
                return "⚠️ Could not retrieve on-call information. The API returned no data."
            
            oncalls = result.get("oncalls") or []
            ...
            for oc in oncalls[:15]:
                if not isinstance(oc, dict):
                    continue
                user = (oc.get('user') or {}).get('summary', 'Unknown')
                schedule = (oc.get('schedule') or {}).get('summary', 'Direct assignment')
                escalation = (oc.get('escalation_policy') or {}).get('summary', '')
                ...
                if key not in seen:
                    ...
                    start = (oc.get('start') or '')[:16].replace('T', ' ')
                    end = (oc.get('end') or '')[:16].replace('T', ' ')
                    if start and end:
                        output += f"  Period: {start} → {end}\n"
                    output += "\n"
    ...
    async def _get_operations_summary(self) -> str:
        """Get overall operations summary with accurate counts from PagerDuty REST API V2.
        
        IMPORTANT: Uses team_ids only (no service_ids) for PagerDuty API v2 compatibility.
        All data extraction is null-safe to handle varied PagerDuty API responses.
        """
        try:
            team_id = get_team_id()
            now = datetime.utcnow()
            ...
            resolved_result = await self.mcp_client.execute_tool("list_incidents", {
                "statuses": ["resolved"],
                "team_ids": [team_id],
                "limit": 100,
            ...
            triggered = (triggered_result.get("incidents") if isinstance(triggered_result, dict) else None) or []
            acknowledged = (ack_result.get("incidents") if isinstance(ack_result, dict) else None) or []
            resolved_all = (resolved_result.get("incidents") if isinstance(resolved_result, dict) else None) or []
            
            # Safe counts — PagerDuty API v2 may return null for 'total'
            triggered_count = len(triggered)
            acknowledged_count = len(acknowledged)
            
            # Count resolved today by last_status_change_at
            resolved_today = sum(
                1 for inc in resolved_all
                if (inc.get("last_status_change_at") or "") >= today_midnight_str
            )
            
            # Get on-call (wrapped in try/except so failure doesn't break summary)
            oncalls = []
            try:
                oncall_result = await self.mcp_client.execute_tool("list_oncalls", {})
                oncalls = (oncall_result.get("oncalls") if isinstance(oncall_result, dict) else None) or []
            except Exception:
                pass
            ...
            output += f"🔴 **Triggered:** {triggered_count}\n"
            output += f"🟡 **Acknowledged:** {acknowledged_count}\n"
            output += f"🟢 **Resolved Today:** {resolved_today}\n"
            output += f"📊 **Total Open:** {triggered_count + acknowledged_count}\n\n"
            ...
            # Show top triggered incidents
            if triggered:
                output += "### ⚡ Top Triggered Incidents\n"
                for inc in triggered[:5]:
                    urgency_emoji = "🔥" if inc.get('urgency') == 'high' else "⚠️"
                    output += f"{urgency_emoji} `{inc.get('id', 'N/A')}` — {(inc.get('title') or 'No title')[:60]}\n"
                output += "\n"
    ...
    async def _get_past_incidents(self, incident_id: str) -> str:
        """Get historical similar incidents using AIOps (requires PagerDuty AIOps feature)."""
        try:
            ...
            if not result or not isinstance(result, dict):
                return f"No past incident data available for `{incident_id}`. This feature may require PagerDuty AIOps."
            ...
            for inc in past_incidents[:10]:
                ...
                created = (inc.get('created_at') or 'N/A')[:16].replace('T', ' ')
                svc = (inc.get('service') or {}).get('summary', 'Unknown Service')
                
                output += f"{status_emoji} **{(inc.get('title') or 'No title')[:60]}**\n"
                output += f"   `{inc.get('id', 'N/A')}` | {inc.get('status', 'unknown')} | {svc} | {created}\n"
                
                if inc.get('resolution'):
                    output += f"   📝 Resolution: {inc.get('resolution')[:150]}\n"
                output += "\n"
            ...
            error_msg = str(e)
            if '404' in error_msg or 'Not Found' in error_msg:
                return f"ℹ️ Past incidents feature is not available for `{incident_id}`. This requires PagerDuty AIOps add-on."
            return f"❌ Error getting past incidents: {error_msg}"
    ...
    async def _chat_api_mode(self, message: str, conversation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Operations summary
            elif any(word in message_lower for word in ['summary', 'overview', 'status overall', 'dashboard', 'how many']):
                response = await self._get_operations_summary()
                tools_used = ["get_operations_summary"]
            
            # Incident details by ID — catch any message with an incident ID pattern
            elif re.search(r'(detail|info|show|get|describe|about|what).*(P[A-Z0-9]{6,})', message, re.IGNORECASE) or \
                 re.search(r'(P[A-Z0-9]{6,}).*(detail|info|status|what)', message, re.IGNORECASE):
            ...
            # Just an incident ID by itself (e.g., user types "PXXXXXX")
            elif re.match(r'^\s*P[A-Z0-9]{6,}\s*$', message, re.IGNORECASE):
                match = re.search(r'P[A-Z0-9]{6,}', message, re.IGNORECASE)
                response = await self._get_incident_details(match.group(0))
                tools_used = ["get_incident_details"]
            
            # Timeline/log entries
            elif 'timeline' in message_lower or 'log entr' in message_lower:
                match = re.search(r'P[A-Z0-9]{6,}', message, re.IGNORECASE)
                if match:
            ...
            # Alerts for a specific incident (must be checked BEFORE generic incident list)
            elif ('alert' in message_lower or 'alerts' in message_lower) and re.search(r'P[A-Z0-9]{6,}', message, re.IGNORECASE):
                match = re.search(r'P[A-Z0-9]{6,}', message, re.IGNORECASE)
                if match:
            ...
            # List incidents — broadened to catch natural language
            elif any(word in message_lower for word in [
                'incident', 'incidents', 'critical', 'triggered',
                'issue', 'issues', 'problem', 'problems',
                'what\'s happening', 'whats happening', 'what is happening',
                'current situation', 'active', 'ongoing',
                'list all', 'show all', 'all open', 'open issues',
                'any alerts', 'any incidents', 'anything going on',
            ...
            elif any(word in message_lower for word in ['past', 'similar', 'history', 'historical']) and 'incident' in message_lower:
                match = re.search(r'P[A-Z0-9]{6,}', message, re.IGNORECASE)
                if match:
            ...
            # Analyze incident
            elif any(word in message_lower for word in ['analyze', 'analysis', 'investigate', 'diagnose', 'root cause']):
                match = re.search(r'P[A-Z0-9]{6,}', message, re.IGNORECASE)
                if match:
                    # For analysis, we get the detailed view which now includes alerts and notes
                    details = await self._get_incident_details(match.group(0))
                    response = f"## 🕵️ AI Analysis for {match.group(0)}\n\n"
                    response += "I've retrieved the full incident context including alerts, custom details, and notes.\n\n"
                    response += details
                    tools_used = ["get_incident_details"]
                else:
                    response = "Please provide an incident ID. Example: 'analyze PXXXXXX'"
            ...
            # Default - show summary with helpful hint
            else:
                response = await self._get_operations_summary()
                response += "\n\n💡 *I wasn't sure what you meant. Here's a summary instead. Type 'help' to see all available commands.*"
                tools_used = ["get_operations_summary"]
